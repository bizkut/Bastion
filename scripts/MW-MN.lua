-- Midnight Expansion Conduit of the Celestials: C4QAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAMWmZZYxixMjNjlltZ4B2Mz222MjFmRzYGwgBMLzMzMMbYGmFTAAAAAEgFbjFmZAAAAgZAjBWkxMA

local Tinkr, Bastion = ...

local RestoMonkModule = Bastion.Module:New('MW-MN')
...
-- SpearHandStrike removed in Midnight
...
-- Conduit of the Celestials Hero Talent Spells
local HeartOfTheJadeSerpent = SpellBook:GetSpell(443294) -- TFT CDR buff
local StrengthOfTheBlackOxBuff = SpellBook:GetSpell(443110) -- Instant Enveloping Mist proc
local CourageOfTheWhiteTiger = SpellBook:GetSpell(443087) -- Damage/heal proc from TP/Vivify
local FlowingWisdomBuff = SpellBook:GetSpell(1262672) -- Haste during HotJS
local ChiJiSwiftness = SpellBook:GetSpell(443566) -- Movement speed after Celestial assist
local Mistline = SpellBook:GetSpell(1280297) -- ReM healing bonus on lowest target
...
local function CondCrackling()
    -- Lightning condition
    if Player:GetPartyHPAround(40, 75) >= 2 or Player:GetPartyHPAround(40, 80) >= 3 then
        return true
    end
    return false
end
...
local function isChannelingSoothingMistOnTarget(unit)
    if Player:GetCastingOrChannelingSpell() == SoothingMist then
        local channelTargetGUID = ObjectCastingTarget(Player:GetOMToken())
        if channelTargetGUID then
            local channelTarget = Bastion.UnitManager:Get(channelTargetGUID)
            if channelTarget and channelTarget:IsValid() and channelTarget:IsUnit(unit) then
                return true
            end
        end
    end
...
local DispelTarget = Bastion.UnitManager:CreateCustomUnit('dispel', function()
    if cachedUnits.dispelTarget and cachedUnits.dispelTarget:IsValid() and not debuffThresholds[cachedUnits.dispelTarget:GetGUID()] then
        ...
        debuffThresholds[cachedUnits.dispelTarget:GetGUID()] = GetTime()
    end
...
local function recentDefensive()
    -- ExpelHarm removed in Midnight
    if (FortifyingBrew:GetTimeSinceLastCastAttempt() < 2)
...
-- Conduit of the Celestials: Check if Heart of the Jade Serpent is active (CDR buff from TFT)
local function IsHeartOfJadeSerpentActive()
    return Player:GetAuras():FindMy(HeartOfTheJadeSerpent):IsUp() or Player:GetAuras():FindMy(FlowingWisdomBuff):IsUp()
end

-- Conduit of the Celestials: Check if Strength of the Black Ox proc is active (instant Enveloping Mist)
local function HasStrengthOfBlackOx()
    return Player:GetAuras():FindMy(StrengthOfTheBlackOxBuff):IsUp()
end

-- Conduit of the Celestials: Should prioritize instant Enveloping Mist for absorb shields
local function ShouldUseStrengthOfBlackOxEnvelope(unit)
    return unit:IsValid() and EnvelopingMist:IsKnownAndUsable() 
        and HasStrengthOfBlackOx()
        and unit:GetAuras():FindMy(EnvelopingMist):IsDown()
        and unit:GetAuras():FindAny(LifeCocoon):IsDown()
        and (unit:GetRealizedHP() < 90 or unit:IsTank())
end
...
local function TFTEnvelope()
    ...
    local shouldUseForChiji = Player:GetAuras():FindMy(ChiJiBuff):IsUp() and envelopeLowestHP < 90 and
        EnvelopeLowest:IsValid() and ShouldUseEnvelopingMist(EnvelopeLowest) and EnvelopeLowest:GetAuras():FindMy(EnvelopingBreath):IsDown()
...
-- SpearHandStrike removed in Midnight - Mistweaver no longer has this ability
-- InterruptAPL:AddSpell(
...
-- DiffuseMagic is now merged into Fortifying Brew in Midnight
-- DefensiveAPL:AddSpell(
...
-- Conduit of the Celestials: Strength of the Black Ox Enveloping Mist priority
-- Cast instant Enveloping Mist on damaged/debuffed targets for absorb shields (Stampede of the Ancients)
DefensiveAPL:AddSpell(
    EnvelopingMist:CastableIf(function(self)
        return self:IsKnownAndUsable()
            and HasStrengthOfBlackOx()
            and EnvelopeLowest:IsValid()
            and ShouldUseStrengthOfBlackOxEnvelope(EnvelopeLowest)
            and EnvelopeLowest:GetRealizedHP() < 85
    end):SetTarget(EnvelopeLowest):OnCast(function(self)
        Bastion.Notifications:AddNotification(EnvelopingMist:GetIcon(), "Strength of the Black Ox - Instant Env")
    end)
)

-- Conduit of the Celestials: Celestial Conduit major healing CD
DefensiveAPL:AddSpell(
    CelestialConduit:CastableIf(function(self)
        return self:IsKnownAndUsable()
            and not Casting:PlayerIsBusy(CelestialConduit)
            and (Player:GetPartyHPAround(40, 65) >= 2 or Player:GetPartyHPAround(40, 70) >= 3)
            and canCastAoE()
    end):SetTarget(Player):PreCast(function()
        isAoEQueuedInThisTick = true
    end):OnCast(function(self)
        Bastion.Notifications:AddNotification(CelestialConduit:GetIcon(), "Celestial Conduit")
    end)
)
...
DefensiveAPL:AddSpell(
    SoothingMist:CastableIf(function(self)
        return self:IsKnownAndUsable() -- and (not Player:IsCastingOrChanneling() or spinningCrane())
            ...
            and Player:GetAuras():FindMy(ThunderFocusTea):IsDown()
            and not stopCasting()
    end):SetTarget(Lowest):OnCast(function(self)
        Bastion.Notifications:AddNotification(SoothingMist:GetIcon(), "Soothing Mist on Lowest")
    end)
)
-- DPS APL
...
RestoMonkModule:Sync(function()
    ...
    -- if Player:GetAuras():FindMy(BalancedStratagem):GetCount() then
    --     print("Balanced Stratagem stacks: " .. Player:GetAuras():FindMy(BalancedStratagem):GetCount())
    -- end
    ...
    if Player:GetCastingOrChannelingSpell() == CracklingJadeLightning and Player:GetAuras():FindMy(JadeEmpowerment):IsDown() and lastSpell and lastSpell:GetID() == CracklingJadeLightning:GetID() then
        _G.SpellStopCasting()
    end
    DispelAPL:Execute()
    ...
    -- OOC manatea
    UpdateManaTeaStacks()
    if (Lowest:GetRealizedHP() > 80) and (ManaTea:GetTimeSinceLastCastAttempt() > 5) and (manaTeaStacks >= 5) then
        if ((Player:GetPP() < 90) and not Player:IsAffectingCombat()) or ((Player:GetPP() < 80) and Player:IsAffectingCombat() and not Player:IsMoving()) then
            manaAPL:Execute()
        end
    end
